 ...
 *
 */
@PermInfo(value = "登录模块",pval="a:auth")  //自定义权限注解 value权限名称 pval权限值
@RestController
@RequestMapping("/auth")
public class AuthController {
     ...
     * 这个登录接口实现了1。存储用户权限信息2。查询出用户权限信息的功能
     * @param body
     * @return
     */
    @PostMapping("/login")
    public Json login(@RequestBody String body){
        ...
        JSONObject json = JSON.parseObject(body);//json字符串转json对象
        ...
        try {
            ...
            //TODO 这一步执行完存入用户权限信息到Subject中后才能取出包含用户权限信息的SysUser对象！！！
            //从Subject中取出用户信息，相当于从从session中取出用户信息
            SysUser user = (SysUser) currentUser.getPrincipal();
            if (user==null) {
                throw new AuthenticationException();
            }
            log.info("登录用户: {}, sessionId: {}",user.getUname(),currentUser.getSession().getId());
